...
(function(window) {
    ...
    DashboardUtils.prototype.exportReportToCSV = function(report, filename) {
        ...
        var headers = ["Control Category", "Control ID", "Description", "Compliance Status"];
    ...
    DashboardUtils.prototype.filterReportsByComplianceStatus = function(reports, status) {
        return reports.filter(function(report) {
            var totalControls = 0;
            var compliantControls = 0;

            ['access_control', 'audit_logging', 'network_security', 
             'configuration_management', 'incident_response'].forEach(function(category) {
                if (report[category]) {
                    report[category].forEach(function(control) {
                        totalControls++;
    ...
    DashboardUtils.prototype.searchReports = function(reports, keyword) {
        ...
        return reports.filter(function(report) {
            // Search in metadata
            if (report.environment && report.environment.toLowerCase().includes(keyword.toLowerCase())) {
                return true;
            }
            if (report.scan_date && report.scan_date.toLowerCase().includes(keyword.toLowerCase())) {
                return true;
            }
            ...
            return controlCategories.some(function(category) {
                if (report[category]) {
                    return report[category].some(function(control) {
                        return (control.control_id && control.control_id.toLowerCase().includes(keyword.toLowerCase())) ||
                               (control.description && control.description.toLowerCase().includes(keyword.toLowerCase()));
                    });
                }
                return false;
            });
