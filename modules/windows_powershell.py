        ...
        try:
            ...
            result = subprocess.run(
                ['powershell.exe', '-EncodedCommand', 
                 subprocess.list2cmdline([encoded_script.decode('utf-16le')])],
                capture_output=True, 
                text=True, 
                shell=True
            )
    ...
    def scan_user_accounts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results.append({
                ...
                'details': {
                    'admin_users': scan_data.get('admin_users', [])
                },
            ...
            results.append({
                ...
                'compliant': len(scan_data.get('disabled_accounts', [])) <= self.security_thresholds['max_disabled_accounts'],
                'details': {
                    'disabled_accounts': scan_data.get('disabled_accounts', [])
                },
            ...
            results.append({
                ...
                'compliant': (
                    password_policy.get('MinPasswordLength', 0) >= self.security_thresholds['password_complexity']['min_length'] and
                    password_policy.get('ComplexityEnabled', False) == self.security_thresholds['password_complexity']['require_complexity']
                ),
                'details': {
