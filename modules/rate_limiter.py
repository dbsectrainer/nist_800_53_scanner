...
class RateLimiter:
            ...
            for limit_name, limit in self.limits.items():
                ...
                for key, state in self.states[limit_name].items():
                    ...
                    if (not state.requests and 
                        (not state.blocked_until or current_time > state.blocked_until)):
    ...
    def get_remaining(self, limit_name: str, key: str) -> Dict[str, int]:
        """
        ...
        with self.lock:
            if (limit_name not in self.states or 
                key not in self.states[limit_name]):
                return {
